...
def show_database_management():
    ...
    if selected_table:
        ...
        primary_key = get_primary_key(selected_table)

        # Suchfunktion
        st.subheader("🔍 Suche")
        search_term = st.text_input("Suchbegriff eingeben", key=f"search_{selected_table}")

        if search_term:
            # Tabelle durchsuchen
            search_results = search_table(selected_table, search_term)
            ...
            try:
                ...
                st.write(f"**Zeige die ersten 100 Einträge**")
                st.dataframe(df, use_container_width=True)
            except Exception as e:
                st.error(f"Fehler beim Abrufen der Daten: {str(e)}")

        # Datensatz bearbeiten
        st.subheader("✏️ Datensatz bearbeiten")
        if primary_key:
            # Primärschlüsselwerte abrufen
            try:
                query = f"SELECT {primary_key} FROM {selected_table} ORDER BY {primary_key}"
                pk_df = pd.read_sql(query, con=engine)
                pk_values = pk_df[primary_key].tolist()

                if pk_values:
                    selected_pk = st.selectbox(f"{primary_key} auswählen", pk_values)

                    if selected_pk:
                        # Datensatz abrufen
                        query = f"SELECT * FROM {selected_table} WHERE {primary_key} = :pk"
                        with engine.connect() as conn:
                            result = conn.execute(text(query), {"pk": selected_pk})
                            row = result.fetchone()

                        if row:
                            # Formular zum Bearbeiten des Datensatzes
                            with st.form(f"edit_{selected_table}_{selected_pk}"):
                                st.write(f"Datensatz mit {primary_key} = {selected_pk} bearbeiten")
                                ...
                                for col in columns:
                                    if col != primary_key:  # Primärschlüssel nicht bearbeiten
                                        col_value = row[col]
                                        values[col] = st.text_input(f"{col}", value=str(col_value) if col_value is not None else "")
                            ...
                            if submit:
                                try:
                                    ...
                                    update_query = text(f"UPDATE {selected_table} SET {set_clause} WHERE {primary_key} = :pk")

                                    # Parameter für die Query
                                    params = {col: val for col, val in values.items()}
                                    ...
                                    st.success(f"Datensatz mit {primary_key} = {selected_pk} erfolgreich aktualisiert!")
                                except Exception as e:
                                    st.error(f"Fehler beim Aktualisieren des Datensatzes: {str(e)}")
                        else:
                            st.warning(f"Datensatz mit {primary_key} = {selected_pk} nicht gefunden.")
                else:
                    st.info(f"Keine Datensätze in der Tabelle {selected_table} gefunden.")
            except Exception as e:
                st.error(f"Fehler beim Abrufen der Primärschlüsselwerte: {str(e)}")
        else:
            st.warning(f"Kein Primärschlüssel für die Tabelle {selected_table} gefunden.")

        # Neuen Datensatz hinzufügen
        st.subheader("➕ Neuen Datensatz hinzufügen")
        with st.expander("Formular anzeigen"):
            ...
            if columns:
                with st.form(f"add_{selected_table}"):
                    st.write(f"Neuen Datensatz in {selected_table} hinzufügen")

                    # Formularfelder für jede Spalte
                    values = {}
                    for col in columns:
                        if col != primary_key or not primary_key:  # Primärschlüssel überspringen, wenn Auto-Increment
                            col_type = column_types.get(col, "").lower()
                            if "int" in col_type:
                                values[col] = st.number_input(f"{col}", step=1)
                            elif "decimal" in col_type or "float" in col_type or "double" in col_type:
                                values[col] = st.number_input(f"{col}", step=0.01)
                            elif "date" in col_type:
                                values[col] = st.date_input(f"{col}")
                            elif "time" in col_type and "datetime" not in col_type:
                                values[col] = st.time_input(f"{col}")
                            elif "datetime" in col_type or "timestamp" in col_type:
                                values[col] = st.date_input(f"{col}")  # Vereinfachung: nur Datum
                            else:
                                values[col] = st.text_input(f"{col}")
                ...
                if submit:
                    try:
                        ...
                        cols = ", ".join([col for col in values.keys() if values[col]])
                        placeholders = ", ".join([f":{col}" for col in values.keys() if values[col]])
                        insert_query = text(f"INSERT INTO {selected_table} ({cols}) VALUES ({placeholders})")

                        # Parameter für die Query
                        params = {col: val for col, val in values.items() if val}

                        # Query ausführen
                        with engine.begin() as conn:
                            conn.execute(insert_query, params)

                        st.success(f"Datensatz erfolgreich zu {selected_table} hinzugefügt!")
                    except Exception as e:
                        st.error(f"Fehler beim Hinzufügen des Datensatzes: {str(e)}")
            else:
                st.warning(f"Keine Spalten für die Tabelle {selected_table} gefunden.")
